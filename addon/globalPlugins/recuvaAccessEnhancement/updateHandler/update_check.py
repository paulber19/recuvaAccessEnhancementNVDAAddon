...
def checkCompatibility(addonName, minimumNVDAVersion=None, lastTestedVersion=None, auto=True):  # noqa:E501
	...
	def checkNVDACompatibility(minimumNVDAVersion, lastTestedNVDAVersion, auto):
		...
		if not auto:
			...
			gui.messageBox(
				# Translators: message to report no compatibility.
				_("The update is not compatible with this version of NVDA. Minimum NVDA version: {minYear}.{minMajor}, last tested: {testedYear}.{testedMajor}.").format(
					minYear=str(minimumYear),
					minMajor=str(minimumMajor),
					testedYear=str(lastTestedYear),
					testedMajor=str(lastTestedMajor)),
